      ...
      <section>
        ...
        <div className="divide-y divide-white/[0.06]">
          {([
            ...
            <SettingsRow key={key} label={label} description={tooltip} displayValue={formData.resources?.[key as keyof typeof formData.resources]} onSave={() => saveSection({ resources: formData.resources }, 'Default resources updated.')}>
              <input
                type="number"
                min="0"
                className="input w-full max-w-md"
        ...
        <div className="divide-y divide-white/[0.06]">
          ...
          {formData.payments?.paypal?.enabled && (
            <>
              ...
              <SettingsRow label="Webhook ID" description={< displayValue={formData.payments?.paypal?.webhookId || 'Not set'} onSave={() => saveSection({ payments: { paypal: formData.payments.paypal } }, 'PayPal settings updated.')}>Configure your PayPal Webhook to POST to <code className="bg-[#202020] px-2 py-1 rounded text-blue-400" displayValue={formData.payments?.paypal?.webhookId || 'Not set'} onSave={() => saveSection({ payments: { paypal: formData.payments.paypal } }, 'PayPal settings updated.')}>{process.env.NEXT_PUBLIC_API_BASE}/api/paypal/webhook</code displayValue={formData.payments?.paypal?.webhookId || 'Not set'} onSave={() => saveSection({ payments: { paypal: formData.payments.paypal } }, 'PayPal settings updated.')}> and paste the Webhook ID here.</ displayValue={formData.payments?.paypal?.webhookId || 'Not set'} onSave={() => saveSection({ payments: { paypal: formData.payments.paypal } }, 'PayPal settings updated.')}>} displayValue={formData.payments?.paypal?.webhookId || 'Not set'} onSave={() => saveSection({ payments: { paypal: formData.payments.paypal } }, 'PayPal settings updated.')}>
                <input
                  type="text"
                  className="input w-full max-w-md"
