...
export default function AdminTicketItem({ t, onAction }:{ t: Ticket; onAction: (action: 'close'|'resolve'|'delete'|'restore'|'reopen', id: string)=>Promise<void> }){
  ...
  return (
    <div className={`relative border-b border-white/[0.06] transition-colors hover:bg-white/[0.015] ${opening ? 'opacity-70' : ''}`}>
      <div className="grid grid-cols-[1fr_auto] items-center gap-3 py-4 md:grid-cols-[1fr_130px_100px_90px_80px_60px_36px] md:gap-4">
          ...
          <div className="flex items-center gap-2.5">
            <div className="min-w-0">
              ...
              <div className="mt-0.5 flex items-center gap-1.5 md:hidden">
                <span className="text-[11px] capitalize text-white/30">{t.user?.username || t.user?.email || 'User'}</span>
                <span className="text-white/10">&middot;</span>
                <span className="text-[11px] text-white/25">{formatRelative(t.updatedAt)}</span>
              </div>
            </div>
          </div>
        </button>
        ...
        {/* User — desktop */}
        <span className="hidden truncate text-xs text-white/35 md:block" title={t.user?.email || t.user?.username}>
          {t.user?.username || t.user?.email || 'User'}
        </span>

        {/* Category — desktop */}
        <span className="hidden text-xs capitalize text-white/25 md:block">
          {t.category || 'general'}
        </span>
