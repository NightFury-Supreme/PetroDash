...
export function EmailDrawer({
  ...
  loginMethod?: string;
  tfaEnabled?: boolean;
  updateEmail: (email: string, pass: string, tfa: string) => Promise<void>;
}) {
  ...
  const emailValidation = useMemo(() => {
    if (!emailDraft) return { valid: false, message: 'Email address is required.' };
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailDraft)) return { valid: false, message: 'Enter a valid email address.' };
    return { valid: true, message: 'Looks good!' };
  }, [emailDraft]);

  const pwValidation = useMemo(() => {
    if (loginMethod !== 'email') return { valid: true, message: '' };
    if (!emailPassword) return { valid: false, message: 'Password is required.' };
    if (emailPassword.length < 6) return { valid: false, message: 'Password must be at least 6 characters.' };
    return { valid: true, message: 'Password confirmed.' };
  }, [emailPassword, loginMethod]);

  const tfaValidation = useMemo(() => {
    if (!tfaEnabled) return { valid: true, message: '' };
    if (!emailTfaCode) return { valid: false, message: useBackupCode ? 'Backup code is required.' : '2FA code is required.' };
    if (useBackupCode && emailTfaCode.length !== 8) return { valid: false, message: 'Enter your 8-character backup code.' };
    if (!useBackupCode && emailTfaCode.length !== 6) return { valid: false, message: 'Enter your 6-digit authenticator code.' };
    return { valid: true, message: useBackupCode ? '8-character backup code entered.' : '6-digit code entered.' };
  }, [emailTfaCode, tfaEnabled, useBackupCode]);

  const isValid = emailValidation.valid && pwValidation.valid && tfaValidation.valid;
      ...
      <div className="grid gap-6 mt-2">
        ...
        <div>
          ...
          <input
            type="email"
            value={emailDraft}
            onChange={(e) => { setEmailDraft(e.target.value); setTouchedEmail(true); }}
        ...
        {loginMethod === 'email' && (
          <div>
            ...
            <input
              type="password"
              value={emailPassword}
              onChange={(e) => { setEmailPassword(e.target.value); setTouchedPw(true); }}
              onBlur={() => setTouchedPw(true)}
              disabled={isLoading}
              className={`w-full h-11 rounded-lg border bg-[#161616] px-4 text-[13px] text-white outline-none focus:border-[#FF5722] transition-colors disabled:opacity-50 ${touchedPw && !pwValidation.valid ? 'border-red-400/30' : 'border-[#222]'}`}
              placeholder="••••••••"
            />
        ...
        {tfaEnabled && (
          <div>
            <div className="flex justify-between items-end mb-2">
              ...
              <button type="button" onClick={() => { setUseBackupCode(!useBackupCode); setEmailTfaCode(''); setTouchedTfa(false); }} className="text-[12px] font-medium text-[#FF5722] hover:text-[#F4511E] transition-colors">
            ...
            <input
              ...
              value={emailTfaCode}
              onChange={(e) => { setEmailTfaCode(e.target.value.replace(useBackupCode ? /[^0-9a-fA-F]/g : /\D/g, '')); setTouchedTfa(true); }}
