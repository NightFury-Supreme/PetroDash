...
export function useServerEdit(serverId: string): UseServerEditReturn {
  ...
  throw new Error(_err);
        ...
        setError(errorMsg);
        if (!errorMsg.toLowerCase().includes('not found') && !errorMsg.toLowerCase().includes('invalid id')) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(errorMsg);
      // Removed console.error(err) to prevent Next.js dev overlay from showing for handled API errors
      return false;
    } finally {
      setSaving(false);
    }
