...
export default function AdminTicketDetailPage() {
  ...
  const updateStatus = async (status: string) => {
    try {
      ...
      if (!r.ok) { let d: any = {}; try { d = await r.json(); } catch {} throw new Error(d?.error || "Failed to update status"); }
  ...
  const updatePriority = async (priority: string) => {
    try {
      ...
      if (!r.ok) { let d: any = {}; try { d = await r.json(); } catch {} throw new Error(d?.error || "Failed to update priority"); }
