...
c = c.replace(/showSuccess\('Email verified successfully'\);/, "showSuccess('Email verified successfully');\n        setTimeout(() => router.replace('/dashboard'), 1500);");

// Remove the verified success state view
c = c.replace(/if \(success && success\.toLowerCase\(\)\.includes\('email verified'\)\) \{[\s\S]*?if \(changeMode\)/, 'if (changeMode)');
