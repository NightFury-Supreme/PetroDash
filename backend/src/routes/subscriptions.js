...
router.get('/', requireAuth, createRateLimiter(30, 60 * 1000), async(req, res) => {
  const list = await Subscription.find({ userId: req.user.sub })
...
router.post('/', requireAuth, createRateLimiter(5, 60 * 1000), async(req, res) => {
  try {
    ...
    if (!plan || !plan.paypalPlanId)
      return res.status(400).json({ error: 'Plan not configured for subscriptions' });
...
router.post('/confirm', requireAuth, createRateLimiter(10, 60 * 1000), async(req, res) => {
  try {
    ...
    const r = await axios.get(
      `${baseUrl}/v1/billing/subscriptions/${encodeURIComponent(sanitizedSubscriptionId)}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    const data = r.data || {};
