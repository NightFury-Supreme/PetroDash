  ...
  async (req, res) => {
    try {
        ...
        if (newName !== server.name) {
          ...
          if (server.panelServerId) {
            try {
              ...
              console.log(
                `[UPDATE_SERVER] Successfully renamed server ${serverId} on Pterodactyl panel`
              );
            } catch (panelError) {
              console.error(
                `[UPDATE_SERVER] Failed to rename server ${serverId} on Pterodactyl panel:`,
                panelError
              );
              console.error(
                `[UPDATE_SERVER] Panel rename error response:`,
                panelError?.response?.data
              );
              return res.status(502).json({
                ...
                panelResponse:
                  process.env.NODE_ENV === 'development' ? panelError?.response?.data : undefined
              });
            }
          }
      ...
      if (limits && typeof limits === 'object') {
        ...
        if (newLimits.memoryMb < min.memoryMb)
          violations.memoryMb = `Minimum memory is ${min.memoryMb} MB`;
        if (newLimits.cpuPercent < min.cpuPercent)
          violations.cpuPercent = `Minimum CPU is ${min.cpuPercent}%`;
        if (newLimits.backups < min.backups)
          violations.backups = `Minimum backups is ${min.backups}`;
        if (newLimits.databases < min.databases)
          violations.databases = `Minimum databases is ${min.databases}`;
        if (newLimits.allocations < min.allocations)
          violations.allocations = `Minimum allocations is ${min.allocations}`;
        ...
        if (server.panelServerId) {
          try {
            ...
            if (!allocationId) {
              ...
              throw new Error(
                'Could not find allocation ID for server. Server may not have any allocations.'
              );
            }
            ...
            console.log(
              `[UPDATE_SERVER] Successfully updated server ${serverId} on Pterodactyl panel`
            );
          } catch (panelError) {
            console.error(
              `[UPDATE_SERVER] Failed to update server ${serverId} on Pterodactyl panel:`,
              panelError
            );
