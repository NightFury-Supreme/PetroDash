...
router.get('/', requireAuth, async(req, res) => {
  try {
    ...
    try {
      ...
      const active = await UserPlan.find({ userId: req.user.sub, status: 'active' })
      ...
      const withFlag = locationsWithData.map(l => ({
        ...l,
        isPlanAllowed:
          !Array.isArray(l.allowedPlans) ||
          l.allowedPlans.length === 0 ||
          l.allowedPlans.some(ap => tokens.has(String(ap)))
      }));
