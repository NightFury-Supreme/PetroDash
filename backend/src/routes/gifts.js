...
router.post('/create', requireAuth, async(req, res) => {
  try {
    ...
    if (coinsNum > 1_000_000)
      return res.status(400).json({ error: 'Coins exceed maximum allowed' });
    ...
    const activeCount = await Gift.countDocuments({
      createdBy: userId,
      source: 'user',
      enabled: true
    });
    ...
    return res
      .status(201)
      .json({ code: gift.code, coins: coinsNum, maxRedemptions: gift.maxRedemptions, validUntil });
  } catch (error) {
  ...
  try {
    ...
    if (!session) {
      ...
      if (!gift || !gift.enabled)
        return res.status(404).json({ error: 'Invalid or disabled code' });
      const now = new Date();
      if (gift.validFrom && now < gift.validFrom)
        return res.status(400).json({ error: 'Code is not active yet' });
      if (gift.validUntil && now > gift.validUntil)
        return res.status(400).json({ error: 'Code has expired' });
      if (gift.maxRedemptions && gift.redeemedCount >= gift.maxRedemptions)
        return res.status(400).json({ error: 'Code redemption limit reached' });
      const alreadyRedeemed = gift.redemptions?.some(
        r => r.user?.toString() === String(authUserId)
      );
      if (alreadyRedeemed)
      ...
      return res.json({
        ...
        appliedPlans,
        user: { coins: user.coins, resources: user.resources }
      });
    }
        ...
        for (const pid of planIds) {
          try {
            ...
            appliedPlans.push({
              planId: String(plan._id),
              name: plan.name,
              subscriptionId: String(sub[0]._id),
              lifetime: isLifetime,
              expiresAt
            });
          } catch (_) {
