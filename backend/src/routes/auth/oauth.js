...
const configurePassport = async () => {
  ...
  if (settings.auth?.discord?.enabled && settings.auth.discord.clientId) {
    const discordRedirectUri =
      settings.auth.discord.redirectUri ||
      `${process.env.FRONTEND_URL?.replace('dashboard', 'api') || process.env.API_BASE_URL || 'http://localhost:4000'}/api/oauth/discord/callback`;
  ...
  if (settings.auth?.google?.enabled && settings.auth.google.clientId) {
    const googleRedirectUri =
      settings.auth.google.redirectUri ||
      `${process.env.FRONTEND_URL?.replace('dashboard', 'api') || process.env.API_BASE_URL || 'http://localhost:4000'}/api/oauth/google/callback`;
